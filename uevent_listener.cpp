...
UeventListener::UeventListener() {
    ...
    if (device_fd_ == -1) {
        std::cout << "Could not open uevent socket";
    }
...
bool UeventListener::ReadUevent(Uevent* uevent) const {
    ...
    if (n <= 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            std::cout << "Error reading from Uevent Fd";
        }
        return false;
    }
    if (n >= UEVENT_MSG_LEN) {
        std::cout << "Uevent overflowed buffer, discarding";
...
void UeventListener::Poll(const ListenerCallback& callback,
    ...
    while (true) {
        ...
        if (nr == 0) return;
        if (nr < 0) {
            std::cout << "poll() of uevent socket failed, continuing";
            continue;
        }
